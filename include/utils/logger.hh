...
ostream &
operator<<(ostream & os, level const & l)
{
  switch (l)
  {
    case DEBUG:
...
struct _ConsoleStream
{
  ...
  _ConsoleStream(Args &&...) : stream(std::cout), has_color(color)
  {
  }
...
{
  template <typename... Args>
  _FileStream(Args &&... args) : stream(std::forward<Args>(args)...), has_color(color)
  {
  }
...
public:
  template <typename... Args>
  _Logger(Args &&... args) : _sp(std::forward<Args>(args)...), _level(level::DEBUG)
  {
  }
...
private:
  ...
  void
  _print(T const & e, Args const &... args)
  {
    _sp.stream << e;
    _print(args...);
  }
...
public:
  template <typename T, typename... Args>
  void
  operator()(level l, T const & e, Args const &... args)
  {
    if (l > _level)
      return;
